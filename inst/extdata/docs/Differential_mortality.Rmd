...
data("weeekly_counts")
cdc<-weekly_counts
cdc %>% filter(name=="All_Cause") %>% group_by(Jurisdiction_of_Occurrence,pop,MMWR_Year) %>% summarise(deaths=sum(value)) %>% mutate(rate=deaths/(pop/10000)) -> cdc
cdc$Year<-cdc$MMWR_Year
cdc$State<-cdc$Jurisdiction_of_Occurrence

cdc %>% mutate(lb=ifelse(State %in% c("West Virginia","Utah", "California", "Florida", "New York", "North Dakota","Texas", "Oklahoma") ,State,"")) %>%
...
ggplot(d,aes(y=tests_per,x=stringency, label=State) ) + geom_text() + geom_smooth(method="lm")
```
